...
public class Calculator {
    ...
    public static String formatResult(String number) {
        ...
        for (int i = 0; i < originalIndexOfDecimal; ++i) {
            ...
            if (!startsWithDecimalPoint(number) &&
                    (i + 1 != originalIndexOfDecimal && number.charAt(0) == ZERO)) {
    ...
    public static char addDigits(char firstDigit, char secondDigit, boolean carryIn) {
        ...
        if (sum > CARRY_CUTOFF) {
            // subtract 10 to get the rightmost digit
            sum -= CARRY_SUBTRACT;
        }
    ...
    public static String add(String firstNumber, String secondNumber) {
        ...
        int numDigitsBeforeDecimal1 = 0;
        int numDigitsBeforeDecimal2 = 0;
        int numDigitsAfterDecimal1 = 0;
        int numDigitsAfterDecimal2 = 0;

        for (int i = 0; i < firstNumber.indexOf(DECIMAL_POINT); ++i) {
            numDigitsBeforeDecimal1++;
        }
        for (int i = 0; i < secondNumber.indexOf(DECIMAL_POINT); ++i) {
            numDigitsBeforeDecimal2++;
        }

        // this variable determines how to align both numbers
        int differenceInDigits = numDigitsBeforeDecimal2 - numDigitsBeforeDecimal1;
        ...
        for (int i = firstNumber.length() - 1; i > firstNumber.indexOf(DECIMAL_POINT); --i) {
            numDigitsAfterDecimal1++;
        }
        for (int i = secondNumber.length() - 1; i > secondNumber.indexOf(DECIMAL_POINT); --i) {
            numDigitsAfterDecimal2++;
        }

        // needs to be recalculated for digits after the decimal
        differenceInDigits = numDigitsAfterDecimal2 - numDigitsAfterDecimal1;
        if (differenceInDigits > 0) {
